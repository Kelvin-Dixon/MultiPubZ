...
Module MultiPubZ
    ...
    Public Sub Main(ByVal args() As String)
        If args.Length > 0 Then ' Check for Filename Supplied
            ...
            Dim SectionToMatch As String = My.Settings.SectionToMatch
            ...
            'Console.WriteLine("SectionToMatch: " & SectionToMatch)
            ...
            If FileIO.FileSystem.FileExists(SrcFile) Then 'Is this a real File?
                Dim RegexString As String = "(?<Publication>.+)-(?<PubDate>\d{4}\d{2}\d{2})-(?<Section>" & SectionToMatch & ")(?<Pages>\d{2}).pdf"
                'Dim Result = Regex.Match(TestFile, "(?<Publication>.+)-(?<PubDate>\d{4}\d{2}\d{2})-(?<Section>Z)(?<Pages>\d{2}).pdf", RegexOptions.IgnoreCase)
                Dim Result = Regex.Match(TestFile, RegexString, RegexOptions.IgnoreCase)
